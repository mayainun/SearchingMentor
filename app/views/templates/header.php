  ...
  <body>
    <nav class="bg-blue-700 border-gray-200 px-6 py-4 ">
      <div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
        ...
        <div class="justify-between items-center" id="mobile-menu-2">
          <ul class="hidden items-center md:flex flex-col md:flex-row font-medium lg:flex-row lg:space-x-8 lg:mt-0">
            <li>
              <a
                href="?controller=home" class="block p-2 text-white border-gray-100 rounded-md hover:bg-gray-50 lg:hover:bg-white lg:border-0 lg:hover:text-blue-700 lg:p-2"
                aria-current="page"
              >
                Beranda
              </a>
            ...
            <li>
            <?php if (isset($data['session'])) : ?>
              <button onclick="confirmLogout(event)" class="text-blue-700 hidden md:block mt-2 bg-white hover:bg-white focus:ring-4 transition hover:scale-[103%] focus:ring-gray-300 hover:text-blue-700 font-medium rounded-lg text-sm px-5 py-1.5 mb-2 mr-2 focus:outline-none">
                Logout
              </button>
              <form id="logoutForm" action="?controller=Login&method=logoutUser" method="POST" style="display: none;">
              </form>
            <?php else : ?>
      ...
      <ul id="navList" class="hidden text-white md:hidden flex flex-col p-5 list-none ml-3">
        <li class="mb-5">
          <a href="?controller=home">Beranda</a>
        </li>
        <li class="mb-5">
          <a href="?controller=about">Tentang Kami</a>
        </li>
        <li class="mb-5">
        ...
        <?php if(isset($data['session'])): ?>
          <li>
            <button onclick="confirmLogout(event)" class="text-white border px-4 bg-blue-700 rounded-md">
              Logout
            </button>
            <form id="logoutFormMobile" action="?controller=Login&method=logoutUser" method="POST" style="display: none;">
            </form>
          </li>
        <?php else: ?>
          <li>
